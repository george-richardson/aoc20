...
func TestRunMasksProgram(t *testing.T) {
	var tests = []struct {
		input    []string
		expected uint64
	}{
	...
	for _, test := range tests {
		actual := runMasksProgram(test.input)
...
func TestDay14_1(t *testing.T) {
	in, err := utils.ReadListOfStrings(2020, 14)
	if err != nil {
		t.Fatal("Error getting input")
	}
	out := runMasksProgram(*in)
	var expected uint64 = 9967721333886
	if out != expected {
...
func TestRunMasksProgramPar2(t *testing.T) {
	var tests = []struct {
		input    []string
		expected int64
	}{
	...
	for _, test := range tests {
		actual := runMasksProgramPar2(test.input)
		if actual != test.expected {
			t.Errorf("actual %v, expected %v", actual, test.expected)
		}
	}
...
func TestDay14_2(t *testing.T) {
	...
	out := runMasksProgramPar2(*in)
	var expected int64 = 4355897790573
	if out != expected {
		t.Errorf("%d does not match expected %d", out, expected)
	}
	t.Log(out)
...
func TestDecodeMask(t *testing.T) {
	var tests = []struct {
		input                                    string
		expectedZeros, expectedOnes, expectedXes int64
	}{
		{
	...
	for i, test := range tests {
		actualZeros, actualOnes, actualXes := decodeMask(test.input)
		if actualZeros != test.expectedZeros {
			t.Errorf("actual zeros %v, expected %v for input %v '%v'", actualZeros, test.expectedZeros, i, test.input)
		}
		if actualOnes != test.expectedOnes {
			t.Errorf("actual ones %v, expected %v for input %v '%v'", actualOnes, test.expectedOnes, i, test.input)
		}
		if actualXes != test.expectedXes {
			t.Errorf("actual xes %v, expected %v for input %v '%v'", actualXes, test.expectedXes, i, test.input)
		}
	}
}
